...
class ToneVisualizer {
  constructor(element, audioSource, config = {}) {
    // Vérification que p5 est disponible globalement
    if (typeof window.p5 === 'undefined') {
      throw new Error('p5.js must be loaded globally before using ToneVisualizer');
    }
    ...
    const {
      ...
      strokeWeight = 2,
      Tone = window.Tone // Accepter Tone comme paramètre ou utiliser window.Tone
    } = config;
    ...
    this.started = true; // Démarrer automatiquement

    // Vérifier que Tone est disponible
    if (!Tone) {
      throw new Error('Tone.js not found. Either:\n1. Pass Tone in config: new ToneVisualizer(element, source, { Tone })\n2. Or expose it globally: window.Tone = Tone');
    }
  ...
  _initP5() {
    // Supprimer les canvas existants dans l'élément
    const existingCanvases = this.element.querySelectorAll('canvas');
